...
└── wiki/             # generated, owned by you
    ...
    ├── projects/     # durable project pages, status, decisions, and history
...
1. Create or update `wiki/projects/<project-slug>.md` as the durable project overview. Keep stable sections for overview, current status, recent changes, decisions, open risks/blockers, active issues, artifacts/links, and source provenance.
2. Use `wiki/projects/<project-slug>/decisions.md` for accepted/rejected plans, architectural decisions, approval outcomes, and reversals when a project has enough decision history to warrant a separate page.
3. Use `wiki/projects/<project-slug>/history.md` for a compact timeline of meaningful project changes. Do not mirror every issue comment.
4. Always cite Paperclip source material by linking back to issue identifiers, document keys, and raw/source pages. Preserve `current_as_of` metadata so readers know when the project page may be stale.
5. Update `wiki/index.md` under Projects and append a `project` log entry to `wiki/log.md`.
